...
def extract_invoice_data(text: str) -> dict:
    ...
    for line in lines[:10]:
        ...
        if (
            len(line) > 3
            and not line.replace(".", "").replace(",", "").replace("$", "").isdigit()
        ):
            # Skip common header words
            if not any(
                skip in line.lower()
                for skip in ["invoice", "receipt", "bill", "tax", "total"]
            ):
                vendor_name = line
                break
    ...
    for line in lines:
        ...
        if any(
            kw in line_lower
            for kw in ["total", "amount due", "balance", "grand total", "total amount"]
        ):
